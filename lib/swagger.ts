...
export const getApiDocs = () => {
  // Read the swagger-schemas.ts file to include the schemas
  const schemasFilePath = path.join(process.cwd(), "lib", "swagger-schemas.ts");
  const schemasContent = fs.readFileSync(schemasFilePath, "utf8");

  // Extract schema definitions from JSDoc comments
  const schemaDefinitions = {};

  // Define common schemas directly
  const commonSchemas = {
    Error: {
      type: "object",
      properties: {
    ...
    CommunityResponse: {
      ...
      properties: {
        ...
        isFounder: {
          type: "boolean",
          description:
            "Whether the current user is the founder of this community",
        },
        isModerator: {
          type: "boolean",
          description:
    ...
    UserVote: {
      ...
      properties: {
        ...
        value: {
          ...
          enum: [-1, 0, 1],
          description:
            "Vote value (1 for upvote, -1 for downvote, 0 for no vote)",
        },
      },
    },
    User: {
  ...
  const spec = createSwaggerSpec({
    definition: {
      openapi: "3.0.0",
      info: {
        title: "Communeo API Documentation",
        version: "1.0.0",
        description:
          "API documentation for the Communeo platform - a community-driven platform with AI-powered features for identifying and rewarding genuine expertise and high-quality contributions.",
        contact: {
          name: "Communeo Support",
          url: "https://communeo.app/support",
        },
      },
      servers: [
        {
          url: process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3002",
          description: "Communeo API server",
        },
      ],
      tags: [
        { name: "Communities", description: "Community-related endpoints" },
